...
The `Button` component is a general-purpose, reusable UI component that provides a consistent look and feel for clickable elements throughout the Time Ruler plugin. It can display either an icon (via the `[Logo](./Logo.md)` component) or textual content.
...
*   **`src`** (optional, `string`): If provided, this string is passed to the `[Logo](./Logo.md)` component to render an icon within the button. The `[Logo](./Logo.md)` component likely maps this string to a specific SVG icon.
...
*   **Rendering**:
    ...
    *   If the `src` prop is provided, it renders the `[Logo](./Logo.md)` component, passing the `src` value to it. This is used for icon-based buttons.
...
*   **`[Logo](./Logo.md)` Component**: If the `src` prop is used, the `Button` component instantiates and renders the `[Logo](./Logo.md)` component to display an icon.
